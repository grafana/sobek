...
`
	...
	go func() {
		...
		expectedValue := 3
		if len(globals) != expectedValue {
			t.Errorf("wrong globals len: %d, expected %d. globals: %v", len(globals), expectedValue, globals)
		} else {
			t.Logf("globals: %v", globals)
		}
		expectedValue = 8
		if len(locals) != expectedValue {
			t.Errorf("wrong locals len: %d, expected %d. locals: %v", len(locals), expectedValue, locals)
		} else {
